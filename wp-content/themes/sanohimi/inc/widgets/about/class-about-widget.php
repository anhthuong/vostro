<?php
/**
 * About Sanohimi widget.
 *
 * @package Sanohimi
 */

if ( ! class_exists( 'Sanohimi_About_Widget' ) ) {

	class Sanohimi_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'sanohimi' );
			$this->widget_id          = 'sanohimi_widget_about';
			$this->widget_name        = esc_html__( 'About Sanohimi', 'sanohimi' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Upload', 'sanohimi' ),
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'sanohimi' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'sanohimi' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline', 'sanohimi' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'sanohimi' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'sanohimi' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, sanohimi_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'sanohimi_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'sanohimi_register_about_widget' );
function sanohimi_register_about_widget() {
	register_widget( 'Sanohimi_About_Widget' );
}
