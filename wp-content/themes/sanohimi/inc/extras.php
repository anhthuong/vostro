 ...
 * @return string
 */
function sanohimi_set_post_meta_value( $value ) {
	$queried_obj = apply_filters( 'sanohimi_queried_object_id', false );
	...
	$curr_opions = 'sanohimi_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * Sidebar position
 */
add_filter( 'theme_mod_sidebar_position', 'sanohimi_set_post_meta_value' );

/**
 * Header container type
 */
add_filter( 'theme_mod_header_container_type', 'sanohimi_set_post_meta_value' );

/**
 * Content container type
 */
add_filter( 'theme_mod_content_container_type', 'sanohimi_set_post_meta_value' );

/**
 * Footer container type
 */
add_filter( 'theme_mod_footer_container_type', 'sanohimi_set_post_meta_value' );
 ...
 * @return string
 */
function sanohimi_set_header_layout_type( $header_layout ) {
	$queried_obj = apply_filters( 'sanohimi_queried_object_id', false );
	$queried_obj = ( ! $queried_obj ) ? get_the_id() : $queried_obj;
	...
	$post_header_layout = get_post_meta( $queried_obj, 'sanohimi_header_layout_type', true );
...
add_filter( 'theme_mod_header_layout_type', 'sanohimi_set_header_layout_type' );
 ...
 * @return string
 */
function sanohimi_render_macros( $string ) {

	$macros = apply_filters( 'sanohimi_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function sanohimi_render_icons( $content ) {
	$icons     = sanohimi_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'sanohimi_render_icons_callback', $content );
}
 ...
 * @return string
 */
function sanohimi_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function sanohimi_get_render_icons_set() {
	return apply_filters( 'sanohimi_render_icons_set', array(
 ...
 * @return string
 */
function sanohimi_render_theme_url( $url ) {
 ...
 */
function sanohimi_get_image_id_by_url( $image_src ) {
	global $wpdb;
 ...
 */
function sanohimi_post_formats_gallery() {
	$size = sanohimi_post_thumbnail_size();
	...
	if ( ! in_array( get_theme_mod( 'blog_layout_type' ), array( 'masonry-2-cols', 'masonry-3-cols' ) ) ) {
		return do_action( 'cherry_post_format_gallery', array(
			'size' => 'sanohimi-thumb-post',
		) );
	}

	$images = sanohimi_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images( false );
 ...
 * @return bool
 */
function sanohimi_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, sanohimi_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function sanohimi_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'sanohimi-thumb-150-150',
		'fullwidth'    => 'sanohimi-thumb-post',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', sanohimi_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', sanohimi_theme()->customizer->get_default( 'blog_featured_image' ) );
 ...
 * @return string
 */
function sanohimi_get_custom_404_bg(){
	$page_404_bg_url = get_theme_mod( 'page_404_bg_image', sanohimi_theme()->customizer->get_default( 'page_404_bg_image' ) );
	$page_404_bg_url = esc_url( sanohimi_render_theme_url( $page_404_bg_url ) );
