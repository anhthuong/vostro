...
class BookingEditCPTPage extends EditCPTPage {
	...
	public function renderRoomsDetailsMetaBox( $post, $metabox ){
		...
		foreach ( $reservedRooms as $key => $reservedRoom ) {
			...
			if ( $room ) {
				echo ' <a href="' . get_edit_post_link( $room->getId() ) . '">';
				echo $room->getTitle();
				echo '</a>';
			} else {
			...
			if ( $rate ) {
				echo ' <a href="' . get_edit_post_link( $rate->getOriginalId() ) . '">';
				echo $rate->getTitle();
				echo '</a>';
			} else {
				self::EMPTY_VALUE_PLACEHOLDER;
			...
			if ( !empty( $reservedServices ) ) {
				...
				foreach ( $reservedServices as $reservedService ) {
					echo '<li>';
					echo '<a href="' . esc_attr( get_edit_post_link( $reservedService->getOriginalId() ) ) . '">';
