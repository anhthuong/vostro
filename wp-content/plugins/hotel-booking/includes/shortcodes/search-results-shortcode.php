...
//		add_action( 'mphb_sc_search_results_before_info', array( '\MPHB\Views\LoopRoomTypeView', '_renderInfoWrapperOpen' ) );
//		add_action( 'mphb_sc_search_results_after_info', array( '\MPHB\Views\LoopRoomTypeView', '_renderInfoWrapperClose' ) );
	...
	private function setupMatchedRoomTypes(){
		...
		if ( $this->sortingMode === self::SORTING_MODE_PRICE ) {
			$roomTypesPriceList = array_combine( $roomTypeDetailsList, array_map( function( $roomTypeDetails ) use($checkInDate, $checkOutDate) {
			...
			usort( $roomTypes, function( $roomType1, $roomType2) use ($checkInDate, $checkOutDate, $roomTypeMinPrices) {
				return $roomTypesPriceList[$roomType1] > $roomTypesPriceList[$roomType2] ? 1 : -1;
			} );
		}

		$this->availableRoomsCount = array_combine( wp_list_pluck( $roomTypeDetailsList, 'id' ), wp_list_pluck( $roomTypeDetailsList, 'count' ) );
	}
	...
	public function renderRecommendation( $roomTypesQuery ){
		...
		if ( !empty( $this->children ) ) {
			$title .= sprintf( _n( ' and %d child', ' and %d children', 'motopress-hotel-booking' ), $this->children );
		}
				...
				foreach ( $recommendation as $roomTypeId => $roomsCount ) {
					...
					<li>
						...
						<div class="mphb-recommendation-item">
							...
							<a href="<?php echo esc_attr( $roomType->getLink() ); ?>" class="mphb-recommendation-item-link" target="_blank">
