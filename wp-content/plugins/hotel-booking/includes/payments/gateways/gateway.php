...
abstract class Gateway {
	...
	protected function getOption( $optionName ){
		...
		$translatableOptions = array( 'title', 'description' );
	...
	protected function paymentCompleted( $payment ){
		...
		if ( !MPHB()->postTypes()->payment()->statuses()->canStatusCompleted( $payment->getStatus() ) ) {
			return false;
		}

		$payment->setStatus( \MPHB\PostTypes\PaymentCPT\Statuses::STATUS_COMPLETED );
	...
	protected function paymentFailed( $payment ){
		...
		if ( !MPHB()->postTypes()->payment()->statuses()->canStatusFailed( $payment->getStatus() ) ) {
			return false;
		}

		$payment->setStatus( \MPHB\PostTypes\PaymentCPT\Statuses::STATUS_FAILED );
		MPHB()->getPaymentRepository()->save( $payment );
	...
	public function registerOptionsFields( &$subTab ){
		...
		$mainGroupFields = array(
			...
			Fields\FieldFactory::create( "mphb_payment_gateway_{$this->id}_enable", array(
				'type'			 => 'checkbox',
				'inner_label'	 => sprintf( __( 'Enable %s', 'motopress-hotel-booking' ), $this->title ),
				'default'		 => $this->getDefaultOption( 'enable' )
			) ),
			Fields\FieldFactory::create( "mphb_payment_gateway_{$this->id}_is_sandbox", array(
				'type'			 => 'checkbox',
			...
			Fields\FieldFactory::create( "mphb_payment_gateway_{$this->id}_title", array(
				'type'			 => 'text',
			...
			Fields\FieldFactory::create( "mphb_payment_gateway_{$this->id}_description", array(
				'type'			 => 'text',
