...
class CheckoutView {
	 ...
	 * @param Entities\RoomType $roomType
	 */
	public static function renderGuestsChooser( $roomType, $roomKey ){
		...
		<p class="mphb-adults-chooser">
			...
			<select name="<?php echo $namePrefix . '[adults]'; ?>" id="<?php echo $idPrefix . '-adults'; ?>">
				<?php
				for ( $i = 1; $i <= $maxAdults; $i++ ) {
					?>
					<option value="<?php echo $i; ?>" <?php selected( $i, $maxAdults ) ?>>
						<?php echo $i; ?>
					</option>
				<?php } ?>
			</select>
		...
		<?php if ( $roomType->getChildrenCapacity() > 0 ) { ?>
			<p class="mphb-children-chooser">
				...
				<select name="<?php echo $namePrefix . '[children]' ?>" id="<?php echo $idPrefix . '-children'; ?>">
					<?php
					for ( $i = 0; $i <= $maxChildren; $i++ ) {
						?>
						<option value="<?php echo $i; ?>" <?php selected( $i, $maxChildren ); ?>>
							<?php echo $i; ?>
						</option>
					<?php } ?>
				</select>
