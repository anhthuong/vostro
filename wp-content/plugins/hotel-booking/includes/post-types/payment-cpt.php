	...
	public function getFieldGroups(){
		...
		$paymentFields = array(
			...
			Fields\FieldFactory::create(
				...
				'default'	 => !empty( $defaults['_mphb_gateway'] ) ? $defaults['_mphb_gateway'] : MPHB()->settings()->payment()->getDefaultGateway(),
				'required'	 => true,
				)
			),
			Fields\FieldFactory::create(
				...
				'default'	 => !empty( $defaults['_mphb_gateway_mode'] ) ? $defaults['_mphb_gateway_mode'] : 'sandbox',
				'required'	 => true,
				)
			),
			Fields\FieldFactory::create(
				...
				'default'	 => !empty( $defaults['_mphb_amount'] ) ? $defaults['_mphb_amount'] : 0,
				'step'		 => 0.01,
				'min'		 => 0,
				'size'		 => 'price',
				'required'	 => true
			...
			Fields\FieldFactory::create(
				...
				'default'	 => !empty( $defaults['_mphb_booking_id'] ) ? $defaults['_mphb_booking_id'] : '',
				)
			),
		);
		...
		$billingInfoFields = array(
			Fields\FieldFactory::create(
				...
				'default'	 => !empty( $defaults['_mphb_first_name'] ) ? $defaults['_mphb_first_name'] : '',
				)
			),
			Fields\FieldFactory::create(
				...
				'default'	 => !empty( $defaults['_mphb_last_name'] ) ? $defaults['_mphb_last_name'] : '',
				)
			),
			Fields\FieldFactory::create(
				'_mphb_email', array(
				...
				'default'	 => !empty( $defaults['_mphb_email'] ) ? $defaults['_mphb_email'] : '',
				)
			),
			Fields\FieldFactory::create(
				'_mphb_phone', array(
				...
				'default'	 => !empty( $defaults['_mphb_phone'] ) ? $defaults['_mphb_phone'] : '',
				)
			),
			Fields\FieldFactory::create(
